...
def _calculate_svd(m: int, C0_inv: np.ndarray, C1: np.ndarray, eigenvectors: np.ndarray, signal: np.ndarray) -> tuple:
    """ Calculate the SVD of the amplitudes corresponding to the DyCA eigenvector-matrix U and the associated matrix V"""
    ...
    if m is not None:
        ...
        U_svd, S_svd, V_svd = None, None, None

    return U_svd, S_svd, V_svd


def _calculate_amplitudes(U_svd: np.ndarray, S_svd: np.ndarray, V_svd: np.ndarray, n: int) -> dict:
    """ Calculate the amplitudes of the DyCA decomposition"""
    # We can only project if n is known
    if n is not None and U_svd is not None and S_svd is not None and V_svd is not None:
        # Truncation of the SVD to n components yields the amplitudes
        amplitudes = U_svd[0:n, 0:n] @ np.diag(S_svd[0:n]) @ V_svd[0:n, :]
    else:
        amplitudes = None
